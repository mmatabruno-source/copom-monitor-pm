...
- [ ] T004 [P] [US2] Criar `tests/unit/test_bcb_client.py`: erro de conexão e status `5xx`/`429` disparam nova tentativa (respeitando `Retry-After` quando presente); status `4xx` permanente (ex.: `404`) não gera nova tentativa (FR-004, D2)
- [ ] T005 [P] [US2] Criar `tests/unit/test_analise.py`: `_chamar_claude` tenta novamente em falha transitória da API da Anthropic e desiste após as tentativas configuradas; `extrair_selic_resultante` e `extrair_secoes_ata` cobertos com casos de sucesso e de formato inesperado/texto vazio (FR-003, FR-008)
...
- [ ] T007 [P] [US2] Em `src/bcb_client.py`, alterar `_get` para distinguir falha permanente (`4xx` exceto `429`) de transitória (erro de conexão, `5xx`, `429`), abortando sem novas tentativas no caso permanente e respeitando o cabeçalho `Retry-After` em `429` quando presente (FR-004, D2)
- [ ] T008 [P] [US2] Em `src/analise.py`, adicionar retry/backoff (mesmo padrão de `bcb_client._get`) em `_chamar_claude` para falhas tipicamente transitórias da API da Anthropic (FR-003, D1)
