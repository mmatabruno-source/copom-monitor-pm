...
def gerar_mensagens_comunicado(texto_bruto, nro_reuniao, data_publicacao, selic_anterior=None):
    ...
    prompt = (
        ...
        "- Regra de capitalização (vale para toda palavra logo após um emoji ou logo após "
        "dois-pontos, em qualquer lugar do texto): se a palavra vem imediatamente depois de "
        "um emoji, começa com letra MAIÚSCULA; se vem imediatamente depois de dois-pontos, "
        "começa com letra minúscula, como em uma continuação de frase (salvo nomes próprios "
        "ou siglas em ambos os casos).\n"
